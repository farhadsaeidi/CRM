...
if DEBUG:
    ...
    # ⚠️ **پورت‌های `vite preview` (۴۱۷۳ به بعد) هم لازم‌اند، نه فقط `npm run dev`.**
    #
    # پراکسیِ Vite وقتی به شکلِ رشته تعریف شود (`'/api': 'http://localhost:8000'`)
    # خودش `changeOrigin: true` می‌گذارد و `Host` را به `localhost:8000` بازنویسی
    # می‌کند. پس جنگو `Origin`ِ مرورگر را با میزبانِ خودش یکی نمی‌بیند و ناچار
    # سراغِ همین فهرست می‌آید؛ اگر پورت اینجا نباشد هر POST با «Origin checking
    # failed» رد می‌شود. دقیقاً همین روی «گفتگوی جدید» دیده شد، وقتی برنامه برای
    # اشتراک از بیلدِ preview روی ۴۱۷۳ سرو می‌شد.
    _DEV_PORTS = [*range(5173, 5184), *range(4173, 4184)]  # dev و preview، هر کدام ۱۱ پورت
